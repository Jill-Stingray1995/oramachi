const KEYS = ['pop1m','shinkansen','coastal','designated','festival','castle','snow_festival','worldheritage','snow','basin','airport','island','capital','noodle','mascot_famous',
...
function calcStars(city){
  ...
  // 都市規模の個別上書き(公式の指標だけでは実感と合わない自治体向け)
  const URBAN_OVERRIDES = {
    '那覇市': 3
  };
  if (city.name in URBAN_OVERRIDES) {
    urban = URBAN_OVERRIDES[city.name];
  }
  ...
  const local = cap(1 + Math.floor(localCount / 2));
...
function sendCorrectionToSheet(entry){
  console.log('[おらマチデバッグ] sendCorrectionToSheet 呼び出し。URL設定:', CORRECTIONS_ENDPOINT_URL || '(空)');
  if(!CORRECTIONS_ENDPOINT_URL){
    console.log('[おらマチデバッグ] URLが空なのでここで終了します');
    return;
  }
  try{
    console.log('[おらマチデバッグ] fetchを実行します:', CORRECTIONS_ENDPOINT_URL);
    ...
    fetch(CORRECTIONS_ENDPOINT_URL, {
      ...
      console.log('[おらマチデバッグ] fetch自体は完了しました(no-corsなので成否は不明)');
    }).catch(err => {
      console.warn('[おらマチデバッグ] fetchがcatchに入りました。エラー内容:', err);
    });
  }catch(e){
    console.warn('[おらマチデバッグ] fetch呼び出し自体で例外発生:', e);
  }
}
