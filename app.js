...
// 質問の「出やすさ」の調整。鉄道系は絞り込み効率が良く選ばれがちなので出にくくし、
// 遊び心のある質問(ご当地ネタ)は絞り込み効率が悪くても出やすくする。
...
const LOW_PRIORITY_PENALTY = 30;
const HIGH_PRIORITY_BONUS = 45;
function priorityAdjust(k, diff){
  if(LOW_PRIORITY_KEYS.has(k)) return diff + LOW_PRIORITY_PENALTY;
  if(HIGH_PRIORITY_KEYS.has(k)) return Math.max(0, diff - HIGH_PRIORITY_BONUS);
  return diff;
}
...
function entropyPick(){
  ...
  const prelim = candidateQuestions.map(k=>{
    const yes = topCities.filter(c=>c.tags[k]).length;
    const no = topCities.length - yes;
    return { k, diff: priorityAdjust(k, Math.abs(yes - no)) };
  ...
  const scored = shortlist.map(k=>{
    ...
    return { k, minimax: priorityAdjust(k, Math.max(worstYes, worstNo)) };
  });
