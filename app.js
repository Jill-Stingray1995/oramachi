...
function mascotSVG(mood){
  ...
  const mouth = {
    normal: `<path d="M56 74 Q59.5 69 63 74 Q66.5 69 70 74" stroke="#2A2620" stroke-width="2.8" fill="none" stroke-linecap="round" stroke-linejoin="round"/>`,
...
// 診断カード用: 既存のtagsだけから「雪国度」「都会度」「ローカル色」を1〜5段階で算出
// (cities.jsonに新しいフィールドを追加せず、既存タグの組み合わせだけで計算する)
function calcStars(city){
  const t = city.tags;
  const cap = n => Math.max(1, Math.min(5, n));
  const snow = cap(1 + (t.snow ? 2 : 0) + (t.nihonkai ? 1 : 0) + (t.tohoku_dialect ? 1 : 0));
  const urban = cap(1 + (t.pop1m ? 2 : 0) + (t.designated ? 1 : 0) + (t.subway ? 1 : 0) + (t.capital ? 1 : 0));
  const localFlavor = [t.festival, t.castle, t.onsen, t.sake, t.rice_region, t.castle_town, t.mascot_famous, t.famous_mountain]
    .filter(Boolean).length;
  const local = cap(1 + Math.floor(localFlavor / 2));
