...
const QUESTIONS = {
  ...
  hiraya_himawari_no_yu: {text:'道の駅「信州平谷」に日帰り温泉「ひまわりの湯」がある？', icon:'♨️'},
  ikusaka_sanseiji: {text:'県立公園の景勝地「山清路」がある？', icon:'🏞️'}
};
...
const QUESTION_HELP = {
  ...
  national_treasure: '文化庁の国指定文化財等データベースを基準に、国宝建造物、または文化庁・所在地自治体が公式に所在を公開している国宝の絵画・彫刻・工芸品・書跡・典籍・古文書・考古資料・歴史資料がある場合です。',
...
function sharePageUrl(){
  const runtime = window.ORAMACHI_RUNTIME;
  if(runtime && runtime.target === 'native' && typeof runtime.publicUrl === 'string'){
    try{
      const configured = new URL(runtime.publicUrl);
      if(configured.protocol === 'https:'){
        configured.hash = '';
        return configured.href;
      }
    }catch(e){ /* 固定設定が不正な場合は、下のWeb URLへ安全に戻す */ }
  }
...
// 1回ごとの上手さは「クリア評価 S〜E」、長期的な全国攻略の実績は
// 「プレイヤーランク G〜S」として分ける。回答速度はどちらの採点にも使わない。
// ブラウザ版の画面は変えず、クリア評価はAndroid版の全国版だけに記録する。
const NATIVE_RANK_STORAGE_KEY = 'oramachi_rank_results_v1';
const NATIVE_RANK_STORAGE_VERSION = 1;
const NATIVE_RANK_RULES_VERSION = 2;
const NATIVE_RANK_HISTORY_LIMIT = 50;
const NATIVE_RANK_GRADES = Object.freeze(['S','A','B','C','D','E']);
const NATIVE_PLAYER_RANK_ART = Object.freeze({
  G:'native-player-rank-g.webp',
  F:'native-player-rank-f.webp',
  E:'native-player-rank-e.webp',
  D:'native-player-rank-d.webp',
  C:'native-player-rank-c.webp',
  B:'native-player-rank-b.webp',
  A:'native-player-rank-a.webp',
  S:'native-player-rank-s.webp',
});
const NATIVE_PLAYER_RANKS = Object.freeze([
  Object.freeze({ grade:'G', minScore:0,  title:'マチあるきビギナー' }),
  Object.freeze({ grade:'F', minScore:10, title:'ご近所ものしり' }),
  Object.freeze({ grade:'E', minScore:20, title:'まちあて見習い' }),
  Object.freeze({ grade:'D', minScore:30, title:'地方探訪家' }),
  Object.freeze({ grade:'C', minScore:45, title:'全国マチめぐり人' }),
  Object.freeze({ grade:'B', minScore:60, title:'自治体博士' }),
  Object.freeze({ grade:'A', minScore:75, title:'おらマチ名人' }),
  Object.freeze({ grade:'S', minScore:90, title:'日本全国マチマスター' }),
]);
...
function nativeRankGradeForScore(score){
  const n = clampNumber(Math.round(Number(score) || 0), 0, 100);
  if(n >= 90) return 'S';
  if(n >= 80) return 'A';
  if(n >= 70) return 'B';
  if(n >= 60) return 'C';
...
function nativeRankQuestionScore(totalQuestions){
  const questions = clampNumber(Math.floor(Number(totalQuestions) || 0), 0, 80);
  if(questions <= 10) return 90;
  if(questions <= 15) return 80;
  if(questions <= 20) return 70;
  if(questions <= 26) return 60;
  if(questions <= 34) return 50;
  return 40;
}

// 現時点で全自治体に揃っている客観データは人口のため、人口順位を11段階に分けて
// 0〜10点を加える。人口が同じ自治体は必ず同じ点になり、データ更新までは値が変わらない。
let nativeMunicipalityDifficultyCache = null;
function nativeMunicipalityDifficultyBonus(city){
  const population = city && city.stats && Number(city.stats.population);
  if(!Number.isFinite(population) || population < 0) return 0;
  if(!nativeMunicipalityDifficultyCache
    || nativeMunicipalityDifficultyCache.source !== CITIES
    || nativeMunicipalityDifficultyCache.length !== CITIES.length){
    const populationCities = CITIES.filter(candidate =>
      candidate && candidate.name !== '東京' && candidate.stats &&
      Number.isFinite(Number(candidate.stats.population)) && Number(candidate.stats.population) >= 0
    );
    const counts = new Map();
    populationCities.forEach(candidate => {
      const value = Number(candidate.stats.population);
      counts.set(value, (counts.get(value) || 0) + 1);
    });
    const bonusByPopulation = new Map();
    let greaterCount = 0;
    Array.from(counts.keys()).sort((a, b) => b - a).forEach(value => {
      bonusByPopulation.set(value, Math.min(10, Math.floor(11 * greaterCount / populationCities.length)));
      greaterCount += counts.get(value);
    });
    nativeMunicipalityDifficultyCache = {
      source:CITIES,
      length:CITIES.length,
      bonusByPopulation,
    };
  }
  return nativeMunicipalityDifficultyCache.bonusByPopulation.get(population) || 0;
}

function nativeMunicipalityDifficultyLevel(city){
  return Math.min(5, Math.floor(nativeMunicipalityDifficultyBonus(city) / 2) + 1);
}

function nativeMunicipalityDifficultyPoints(cityOrLevel){
  const level = typeof cityOrLevel === 'number'
    ? clampNumber(Math.floor(cityOrLevel), 1, 5)
    : nativeMunicipalityDifficultyLevel(cityOrLevel);
  return [0, 10, 15, 25, 40, 60][level];
}

// 難しい自治体は、クリア評価を最大1段階ほど押し上げる。
function nativeClearDifficultyBonus(level){
  return [0, 0, 1, 3, 5, 10][clampNumber(Math.floor(Number(level) || 1), 1, 5)];
}

function calculateNativeRankResult(city, totalQuestions, answerLogSnapshot, context){
  const questions = clampNumber(Math.floor(Number(totalQuestions) || 0), 0, 80);
  const timings = (Array.isArray(answerLogSnapshot) ? answerLogSnapshot : [])
    .map(entry => entry && entry.responseMs)
    .filter(value => Number.isFinite(value) && value >= 0 && value <= 30000);
  const elapsedMs = timings.length > 0
    ? Math.round(timings.reduce((sum, value) => sum + value, 0))
    : null;
  const questionScore = nativeRankQuestionScore(questions);
  const difficultyLevel = nativeMunicipalityDifficultyLevel(city);
  const difficultyBonus = nativeClearDifficultyBonus(difficultyLevel);
  const totalScore = clampNumber(questionScore + difficultyBonus, 0, 100);
  const details = context && typeof context === 'object' ? context : {};
  const unknownCount = (Array.isArray(answerLogSnapshot) ? answerLogSnapshot : [])
    .reduce((count, entry) => count + (entry && entry.val === null ? 1 : 0), 0);
  return {
    playedAt: new Date().toISOString(),
    roomId: null,
    cityId: cityId(city),
    cityName: displayName(city),
    pref: city.pref,
    mode: 'all',
    success: true,
    ...
    questionScore,
    timeScore: 0,
    populationDifficultyBonus: nativeMunicipalityDifficultyBonus(city),
    difficultyLevel,
    difficultyPoints: nativeMunicipalityDifficultyPoints(difficultyLevel),
    difficultyBonus,
    totalScore,
    grade: nativeRankGradeForScore(totalScore),
    wrongGuessCount: clampNumber(Math.floor(Number(details.wrongGuessCount) || 0), 0, 3),
    unknownCount,
    firstClear: details.firstClear === true,
    personalBest: details.personalBest === true,
    detailsAvailable: true,
    rankRulesVersion: NATIVE_RANK_RULES_VERSION,
    citiesVersion: typeof CITIES_VERSION === 'string' ? CITIES_VERSION : '',
  };
}

function normalizeNativeRankRecord(raw){
  if(!raw || typeof raw !== 'object' || raw.mode !== 'all' || raw.success !== true) return null;
  // 旧版(回答速度を含むA〜G方式)も捨てず、質問数と自治体難易度だけで再評価する。
  if(raw.rankRulesVersion !== 1 && raw.rankRulesVersion !== NATIVE_RANK_RULES_VERSION) return null;
  if(typeof raw.cityId !== 'string') return null;
  const city = CITIES.find(candidate => candidate.name !== '東京' && cityId(candidate) === raw.cityId);
  if(!city) return null;
  const questionCount = Number(raw.questionCount);
  if(!Number.isInteger(questionCount) || questionCount < 0 || questionCount > 80) return null;
  const elapsedMs = raw.elapsedMs == null ? null : Number(raw.elapsedMs);
  if(elapsedMs != null && (!Number.isFinite(elapsedMs) || elapsedMs < 0 || elapsedMs > 80 * 30000)) return null;
  const playedAtMs = Date.parse(raw.playedAt);
  if(!Number.isFinite(playedAtMs)) return null;
  const questionScore = nativeRankQuestionScore(questionCount);
  const difficultyLevel = nativeMunicipalityDifficultyLevel(city);
  const difficultyBonus = nativeClearDifficultyBonus(difficultyLevel);
  const totalScore = clampNumber(questionScore + difficultyBonus, 0, 100);
  const detailsAvailable = raw.rankRulesVersion === NATIVE_RANK_RULES_VERSION && raw.detailsAvailable !== false;
  return {
    ...
    totalScore,
    grade: nativeRankGradeForScore(totalScore),
    wrongGuessCount: detailsAvailable ? clampNumber(Math.floor(Number(raw.wrongGuessCount) || 0), 0, 3) : null,
    unknownCount: detailsAvailable ? clampNumber(Math.floor(Number(raw.unknownCount) || 0), 0, 80) : null,
    firstClear: detailsAvailable && raw.firstClear === true,
    personalBest: detailsAvailable && raw.personalBest === true,
    detailsAvailable,
    rankRulesVersion: NATIVE_RANK_RULES_VERSION,
    citiesVersion: typeof raw.citiesVersion === 'string' ? raw.citiesVersion : '',
  };
}

function emptyNativeRankData(){
  return { version: NATIVE_RANK_STORAGE_VERSION, results: [] };
}

function loadNativeRankData(){
  try{
    const raw = localStorage.getItem(NATIVE_RANK_STORAGE_KEY);
    if(!raw) return emptyNativeRankData();
    const parsed = JSON.parse(raw);
    if(!parsed || parsed.version !== NATIVE_RANK_STORAGE_VERSION || !Array.isArray(parsed.results)){
      return emptyNativeRankData();
    }
    return {
      version: NATIVE_RANK_STORAGE_VERSION,
      results: parsed.results
        .map(normalizeNativeRankRecord)
        .filter(Boolean)
        .sort((a, b) => b.playedAt.localeCompare(a.playedAt))
        .slice(0, NATIVE_RANK_HISTORY_LIMIT),
    };
  }catch(e){
    console.warn('おらマチ: クリア評価記録の読み込みに失敗しました', e);
    return emptyNativeRankData();
  }
}
...
function recordNativeRankResult(result){
  const normalized = normalizeNativeRankRecord(result);
  if(!normalized) return null;
  const data = loadNativeRankData();
  data.results.unshift(normalized);
  data.results = data.results.slice(0, NATIVE_RANK_HISTORY_LIMIT);
  saveNativeRankData(data);
  return normalized;
}

function bestNativeRankResult(data){
...
function roundNativeRankScore(value){
  return Math.round((Number(value) || 0) * 10) / 10;
}

function nativeLogProgress(value, maximum, weight){
  const current = clampNumber(Number(value) || 0, 0, maximum);
  if(current <= 0 || maximum <= 0) return 0;
  return weight * Math.log1p(current) / Math.log1p(maximum);
}

function nativeQuestionEfficiencyCredit(minQuestions){
  const questions = Number(minQuestions);
  if(!Number.isFinite(questions) || questions <= 0) return 0;
  return clampNumber((50 - questions) / 40, 0, 1);
}

function nativePlayerRankForScore(score){
  const current = clampNumber(Number(score) || 0, 0, 100);
  for(let index = NATIVE_PLAYER_RANKS.length - 1; index >= 0; index--){
    if(current >= NATIVE_PLAYER_RANKS[index].minScore) return NATIVE_PLAYER_RANKS[index];
  }
  return NATIVE_PLAYER_RANKS[0];
}

function nativePlayerRankIndex(grade){
  return Math.max(0, NATIVE_PLAYER_RANKS.findIndex(rank => rank.grade === grade));
}

// 通常プレイでは、初クリア自治体・自己ベスト・到達都道府県は減らないため、
// この点数も下がらない。同じ自治体を繰り返しても制覇数と難易度は1回分だけ。
function calculateNativePlayerRank(conquestData){
  const allCities = CITIES.filter(city => city && city.name !== '東京');
  const cityById = new Map(allCities.map(city => [cityId(city), city]));
  const rawEntries = conquestData && conquestData.entries && typeof conquestData.entries === 'object'
    ? Object.values(conquestData.entries)
    : [];
  const entries = rawEntries.filter(entry => entry && typeof entry.id === 'string' && cityById.has(entry.id));
  const totalCities = allCities.length || 1;
  const totalDifficultyCredit = allCities.reduce(
    (sum, city) => sum + nativeMunicipalityDifficultyPoints(city), 0
  ) || 1;
  const difficultyCredit = entries.reduce(
    (sum, entry) => sum + nativeMunicipalityDifficultyPoints(cityById.get(entry.id)), 0
  );
  const efficiencyCredit = entries.reduce(
    (sum, entry) => sum + nativeQuestionEfficiencyCredit(entry.minQuestions), 0
  );
  const reachedPrefectures = new Set(entries.map(entry => cityById.get(entry.id).pref));

  const conquestScoreRaw = nativeLogProgress(entries.length, totalCities, 40);
  const difficultyScoreRaw = nativeLogProgress(difficultyCredit, totalDifficultyCredit, 30);
  const efficiencyScoreRaw = nativeLogProgress(efficiencyCredit, totalCities, 20);
  const reachScoreRaw = 10 * Math.sqrt(clampNumber(reachedPrefectures.size / 47, 0, 1));
  const totalScore = roundNativeRankScore(clampNumber(
    conquestScoreRaw + difficultyScoreRaw + efficiencyScoreRaw + reachScoreRaw, 0, 100
  ));
  const rank = nativePlayerRankForScore(totalScore);
  const rankIndex = nativePlayerRankIndex(rank.grade);
  const nextRank = NATIVE_PLAYER_RANKS[rankIndex + 1] || null;
  const nextRemaining = nextRank ? roundNativeRankScore(Math.max(0, nextRank.minScore - totalScore)) : 0;
  const progress = nextRank
    ? clampNumber(100 * (totalScore - rank.minScore) / (nextRank.minScore - rank.minScore), 0, 100)
    : 100;
  const components = {
    conquest: { label:'自治体クリア数', score:roundNativeRankScore(conquestScoreRaw), maximum:40 },
    difficulty: { label:'自治体の難易度', score:roundNativeRankScore(difficultyScoreRaw), maximum:30 },
    efficiency: { label:'質問効率', score:roundNativeRankScore(efficiencyScoreRaw), maximum:20 },
    reach: { label:'到達都道府県', score:roundNativeRankScore(reachScoreRaw), maximum:10 },
  };
  const weakestKey = Object.keys(components).sort((a, b) =>
    (components[a].score / components[a].maximum) - (components[b].score / components[b].maximum)
  )[0];
  const advice = {
    conquest:'まだ当てていない自治体に挑戦すると伸びやすいです。',
    difficulty:'人口の少ない難しいマチを新しく制覇してみましょう。',
    efficiency:'制覇済みの自治体で自己ベストを更新してみましょう。',
    reach:'まだ到達していない都道府県のマチに挑戦しましょう。',
  }[weakestKey];
  return {
    score: totalScore,
    grade: rank.grade,
    ...
    nextRank,
    nextRemaining,
    progress: roundNativeRankScore(progress),
    distinctCities: entries.length,
    reachedPrefectures: reachedPrefectures.size,
    difficultyCredit,
    efficiencyCredit: roundNativeRankScore(efficiencyCredit),
    components,
    advice,
  };
}
...
function applyImportedConquest(importedConquest, mode){
  ...
  try{
    ...
    saveConquest(result);
    if(isNativeAppRuntime()) syncNativePlayerRankHeader(calculateNativePlayerRank(result));
...
// Android版の「47都道府県」から使う共通定義。
// 47個の画面・処理を複製せず、同じゲーム処理へ都道府県だけを渡す。
// 後方の訂正フォームでも同じ並びを再利用するため、この配列を唯一の正本にする。
// 配列番号は途中ゲームへ保存する mode ID（prefecture-01〜47）にも使うため、
// 既存項目の並び替えはしないこと。都道府県コード順で固定する。
const PREFECTURE_MODE_PREFS = Object.freeze([
...
const PREFECTURE_MODE_PREFIX = 'prefecture-';
function prefectureModeId(pref){
  const index = PREFECTURE_MODE_PREFS.indexOf(pref);
  return index >= 0 ? `${PREFECTURE_MODE_PREFIX}${String(index + 1).padStart(2, '0')}` : '';
}
...
const MODES = {
  ...
  chugokuShikoku: { label: '中国・四国版', regions: ['中国','四国'] }
};

PREFECTURE_MODE_PREFS.forEach((pref, index) => {
  const id = `${PREFECTURE_MODE_PREFIX}${String(index + 1).padStart(2, '0')}`;
  MODES[id] = {
    label: `${pref}版`,
    description: `${pref}の自治体だけで当てます`,
    prefecture: pref
  };
});

function prefectureForMode(mode){
  const definition = MODES[mode];
  return definition && typeof definition.prefecture === 'string'
...
function getModeCities(mode){
  ...
  if(prefecture){
    // 「東京」(都全体の集計エントリ)は23区・市町村の個別データと重複するため除外する。
    // 旧niigata/tokyo IDもこの共通処理へ通し、途中ゲームとの互換性を保つ。
    return CITIES.filter(c => c.pref === prefecture && c.name !== '東京');
  }
  if(MODES[mode] && Array.isArray(MODES[mode].regions)){
    return CITIES.filter(c => MODES[mode].regions.includes(regionOf(c.pref)) && c.name !== '東京');
  }
...
function modeAllowsGroup(mode, group){
  ...
  const prefecture = prefectureForMode(mode);
  if(prefecture){
    // 都道府県版でも、所属地方の質問・路線を使う。
    // 新潟県と東京都には従来から専用質問があるため、それも引き続き有効にする。
    if(group === 'niigata' && prefecture === '新潟県') return true;
    if(group === 'tokyo' && prefecture === '東京都') return true;
    const groupMode = MODES[group];
    if(groupMode && groupMode.region === regionOf(prefecture)) return true;
  }
  const definition = MODES[mode];
  const groupMode = MODES[group];
  if(definition && Array.isArray(definition.regions) && groupMode && groupMode.region){
    if(definition.regions.includes(groupMode.region)) return true;
  }
...
function activeKeysForMode(mode){
  return KEYS.filter(k => {
    ...
    if(IC_ROUTE_GROUPS[k]){
      ...
      return IC_ROUTE_GROUPS[k].some(group => modeAllowsGroup(mode, group));
    }
    // 同じ質問が複数地方に属する場合があるため、最初に見つかった地方だけで
    // 判定を終えず、所属する地方のどれか一つでも現在のモードと合えば有効にする。
    // 例: jra_training は関東と近畿の両方に施設がある。
    const allowedGroups = Object.keys(MODE_ONLY_KEYS)
      .filter(group => MODE_ONLY_KEYS[group].includes(k));
    if(allowedGroups.length > 0){
      return allowedGroups.some(group => modeAllowsGroup(mode, group));
    }
...
const TAG_GAME_CATEGORY = {
  ...
  "yama_in_name": "自治体名",
  "yosan_line": "交通",
  "aogashima_double_caldera": "地理",
  "castle_hyakumeijo": "歴史・文化",
  "circuit": "観光・娯楽",
  "dam_lake": "地理",
  ...
  "national_park": "地理",
  "neba_sugi": "産業",
  "ogata_reclamation": "歴史・文化",
  "oi_biotopia": "観光・娯楽",
  "omocha_no_machi": "産業",
  "ora_mirai_tower": "観光・娯楽",
  "ramsar": "地理",
  "tanada_hyakusen": "地理",
...
function priorityAdjust(k, diff, poolSize, isDecisive, groupShares){
  ...
  if(boostGroup && poolSize <= REGION_KEY_BOOST_POOL_SIZE){
    ...
    const regionFocused = modeAllowsGroup(currentMode, boostGroup) ||
  ...
  if(TOKYO_RAIL_KEYS.has(k) && poolSize <= TOKYO_RAIL_POOL_SIZE){
    ...
    const tokyoFocused = (prefectureForMode(currentMode) === '東京都') ||
...
function isNativeAppRuntime(){
  const runtime = window.ORAMACHI_RUNTIME;
  return Boolean(runtime && runtime.target === 'native');
}
function syncNativeShellFromNavState(state){
  ...
  if(!shell || typeof shell.setState !== 'function'){
    window.ORAMACHI_PENDING_NATIVE_STATE = { tab, immersive };
    return;
  }
  shell.setState(tab, immersive);
}
...
function rememberExactNavState(state){
  ...
  syncNativeShellFromNavState(currentNavState);
  return currentNavState;
}
...
function pushNavState(screen, extra){
  ...
  try{
    if(previous.oramachiModalGuard){
      ...
      syncNativeShellFromNavState(next);
    ...
    if(previous.oramachiScreen === 'game' && previous.gamePhase === 'result' && screen !== 'game'){
      ...
      syncNativeShellFromNavState(sameDepthState);
...
function renderNavStateFromHistory(state){
  ...
  else if(st.oramachiScreen === 'nativePlayHub'){ renderNativePlayHub(); }
  else if(st.oramachiScreen === 'nativePrefecturePicker'){ renderNativePrefecturePicker(); }
...
function nativeDifficultyStars(level){
  const count = clampNumber(Math.floor(Number(level) || 1), 1, 5);
  ...
  stage.innerHTML = `
    <section class="native-home-status" aria-label="現在のプレイ記録">
      <div class="native-stat-card">
        ...
        <span class="native-stat-value">
          <svg class="native-stat-icon" aria-hidden="true" viewBox="0 0 24 24"><path d="M7.3 7h9.4c2.2 0 3.7 1.4 4.2 3.6l1 4.5c.5 2.3-.4 3.9-2.1 3.9-1.1 0-1.8-.6-2.9-2l-.8-1H7.9l-.8 1c-1.1 1.4-1.8 2-2.9 2-1.7 0-2.6-1.6-2.1-3.9l1-4.5C3.6 8.4 5.1 7 7.3 7Zm.2 3.1H6v1.5H4.5v1.5H6v1.5h1.5v-1.5H9v-1.5H7.5v-1.5Zm8.6.7a1.1 1.1 0 1 0 0 2.2 1.1 1.1 0 0 0 0-2.2Zm2.6 2.4a1.1 1.1 0 1 0 0 2.2 1.1 1.1 0 0 0 0-2.2Z"/></svg>
          <strong>${Number(stats.totalPlays || 0).toLocaleString('ja-JP')}</strong><small>回</small>
        </span>
      </div>
      <div class="native-stat-card">
        <span class="native-stat-label">正解</span>
        <span class="native-stat-value">
          <svg class="native-stat-icon" aria-hidden="true" viewBox="0 0 24 24"><path d="M12 2a10 10 0 1 0 9.8 8h-3.1a7 7 0 1 1-4.7-4.7V2.2A10 10 0 0 0 12 2Zm7.8.8-3.3.7-2.2 2.2.4 2-4.2 4.2-1.4-1.4-2.1 2.1 3.5 3.5 6.3-6.3 2 .4L21 6.5l.8-3.7-2 .4Z"/></svg>
          <strong>${Number(stats.totalCorrect || 0).toLocaleString('ja-JP')}</strong><small>回</small>
        </span>
      </div>
      <div class="native-stat-card native-stat-card-conquest">
        <span class="native-stat-label">制覇</span>
        <span class="native-stat-value">
          <svg class="native-stat-icon" aria-hidden="true" viewBox="0 0 24 24"><path d="M5 2h2v20H5V2Zm3 2h10.2l-1.8 3 1.8 3H8V4Z"/></svg>
          <strong>${conqueredCount.toLocaleString('ja-JP')}</strong><small>マチ</small>
        </span>
      </div>
    </section>
    <section class="native-home-hero">
      <div class="mascot-wrap opening-mascot-wrap">${openingMascotHTML()}</div>
      <p class="native-kicker">市区町村まちあてゲーム</p>
      <h1 class="catch-copy" id="catchCopy">あなたの地元、<br class="catch-copy-br">おらっちが当てます。</h1>
      <p class="native-home-lead">「はい」「いいえ」で答えるだけ。日本全国${totalCount.toLocaleString('ja-JP')}市区町村から推理します。</p>
    </section>
    <section class="native-home-rank-strip" aria-label="全国版のクリア評価">
      <div><span>最高<br>クリア評価</span><strong class="native-rank-text native-rank-${bestRank ? bestRank.grade.toLowerCase() : 'none'}">${bestRank ? bestRank.grade : '—'}</strong></div>
      <div><span>直近の<br>クリア評価</span><strong class="native-rank-text native-rank-${latestRank ? latestRank.grade.toLowerCase() : 'none'}">${latestRank ? latestRank.grade : '—'}</strong></div>
      <button type="button" onclick="renderNativeRankHistory()">評価履歴を見る <span aria-hidden="true">›</span></button>
    </section>
    ${resumeCardHtml}
    ...
    <div class="native-primary-actions">
      <button class="native-home-action native-home-action-solo" onclick="startMode('all')">
        ...
        <span class="native-home-action-copy"><strong>ひとりで遊ぶ</strong><small>全国版でクリア評価S〜Eに挑戦</small></span>
        <span class="native-home-action-arrow" aria-hidden="true">›</span>
      </button>
      <button class="native-home-action native-home-action-choose" onclick="renderNativePlayHub()">
        <span class="native-home-action-icon" aria-hidden="true">
...
function renderNativePlayHub(){
  ...
  const areaModes = [
    ...
    { id:'chubu', label:'中部', tone:'chubu' },
    { id:'kinki', label:'近畿', tone:'kinki' },
    { id:'chugokuShikoku', label:'中国・四国', tone:'chugoku-shikoku' },
    { id:'kyushu', label:'九州・沖縄', tone:'kyushu' }
  ];
  stage.innerHTML = `
    <header class="native-play-heading">
      <button class="native-play-back" type="button" onclick="navigateBackOr(renderOpening)" aria-label="ホームへ戻る">‹</button>
      <div class="native-play-heading-copy">
        <span>好きなモードとエリアを選ぼう</span>
        <h1>遊び方を選ぶ</h1>
        <p>おらっちと、あなたのペースでマチあてを楽しもう！</p>
      </div>
      <div class="native-play-heading-mascot" aria-hidden="true">${mascotSVG('happy')}</div>
    </header>
    ${renderGameResumeCardHtml()}
    <section class="native-play-section" aria-labelledby="nativePlayModeTitle">
      <div class="native-play-section-title">
        <span class="native-play-section-icon" aria-hidden="true">🎮</span>
        <h2 id="nativePlayModeTitle">プレイモード</h2>
      </div>
      <div class="native-play-mode-list">
        <button class="native-play-mode-card native-play-mode-solo" type="button" onclick="focusNativeAreaSection()" aria-describedby="nativeSoloDescription">
        ...
        <button class="native-play-mode-card native-play-mode-challenge" type="button" onclick="startChallengeMode()">
          <span class="native-play-mode-icon" aria-hidden="true">🏆</span>
          <span class="native-play-mode-copy"><strong>挑戦状モード</strong><small>ヒントを頼りに、おらっちが選んだ<br>マチをあなたが当てます</small></span>
          <span class="native-play-mode-arrow" aria-hidden="true">›</span>
        </button>
      </div>
    </section>
    <section class="native-play-section native-area-section" id="nativeAreaSection" aria-labelledby="nativeAreaTitle">
      <div class="native-play-section-title">
        <span class="native-play-section-icon native-area-pin" aria-hidden="true">📍</span>
        <div><h2 id="nativeAreaTitle">エリアを選ぶ</h2><p>全国版のほか、地域ごとの限定版も遊べます。</p></div>
      </div>
      <div class="native-area-card">
        <div class="native-area-buttons">
          ${areaModes.map(area => `
            <button class="native-area-button native-area-${area.tone}" type="button" onclick="startMode('${area.id}')">
              <strong>${area.label}</strong><small>${getModeCities(area.id).length.toLocaleString('ja-JP')}</small>
            </button>`).join('')}
          <button class="native-area-button native-area-prefectures" type="button" onclick="renderNativePrefecturePicker()">
            <strong>47都道府県</strong><small>県別に選ぶ ›</small>
          </button>
        </div>
        <div class="native-area-map-panel">
          ${buildNativeAreaMapSvg()}
          <p><strong>遊びたい地域を</strong><span>選んでね！</span></p>
        </div>
      </div>
      <div class="native-area-note"><span aria-hidden="true">✓</span> エリアはゲームごとにいつでも変更できます</div>
    </section>
    <section class="native-play-point-card">
      <strong>遊べるマチは日本全国</strong>
      <span>${totalCount.toLocaleString('ja-JP')}市区町村に対応しています</span>
    </section>
  `;
  footEl.textContent = `対応エリア 日本全国 ${totalCount.toLocaleString('ja-JP')} 市区町村`;
  scrollToPageTop();
}

function focusNativeAreaSection(){
  const area = document.getElementById('nativeAreaSection');
  if(!area) return;
...
function buildNativeAreaMapSvg(){
  const paths = japanMapPaths();
  const names = Object.keys(paths);
  if(names.length === 0){
    return '<div class="native-area-map-fallback" aria-label="日本地図">🗾</div>';
  }
  const mapData = window.ORAMACHI_JAPAN_MAP || {};
  const [vx, vy, vw, vh] = mapData.viewBox || JAPAN_MAP_VIEWBOX;
  const colors = {
    '北海道':'#87bce5', '東北':'#75c3b7', '関東':'#80bd79', '中部':'#e8c665',
    '近畿':'#e8a96f', '中国':'#d899aa', '四国':'#aa8ac0', '九州・沖縄':'#d190b5'
  };
  const shapes = names.map(pref => `<path d="${paths[pref]}" fill="${colors[regionOf(pref)] || '#d8d3c6'}"/>`).join('');
  return `<svg class="native-area-map" viewBox="${vx} ${vy} ${vw} ${vh}" xmlns="http://www.w3.org/2000/svg" aria-hidden="true" focusable="false" preserveAspectRatio="xMidYMid meet"><g stroke="#fffdf6" stroke-width="1.05" stroke-linejoin="round">${shapes}</g></svg>`;
}

function renderNativePrefecturePicker(){
  if(!isNativeAppRuntime()) return renderOpening();
  stopOpeningMascotAnimation();
  activeGameTransientScreen = null;
  currentGameFlowKind = null;
  currentGameFlowPhase = null;
  stampsEl.innerHTML = '';
  if(currentNavState.oramachiScreen === 'nativePrefecturePicker') replaceNavState('nativePrefecturePicker');
  else pushNavState('nativePrefecturePicker');
  const regionOrder = ['北海道','東北','関東','中部','近畿','中国','四国','九州・沖縄'];
  const regionSections = regionOrder.map(region => {
    const prefectures = PREFECTURE_MODE_PREFS.filter(pref => regionOf(pref) === region);
    return `
      <section class="native-prefecture-region">
        <h2>${region}</h2>
        <div class="native-prefecture-grid">
          ${prefectures.map(pref => {
            const mode = prefectureModeId(pref);
            return `<button type="button" onclick="startMode('${mode}')"><span>${pref}</span><small>${getModeCities(mode).length}自治体</small><b aria-hidden="true">›</b></button>`;
          }).join('')}
        </div>
      </section>`;
  }).join('');
...
function renderNativeRecordHub(){
  ...
  const correctRate = stats.totalPlays > 0
  ...
  stage.innerHTML = `
    ...
    <div class="native-metric-grid native-record-metrics">
      <div class="native-metric"><strong>${Number(stats.totalPlays || 0).toLocaleString('ja-JP')}</strong><span>プレイ</span></div>
      <div class="native-metric"><strong>${Number(stats.totalCorrect || 0).toLocaleString('ja-JP')}</strong><span>正解</span></div>
      <div class="native-metric"><strong>${correctRate}%</strong><span>正解率</span></div>
    </div>
    <section class="native-hub-section">
      <h2>コレクション</h2>
      <div class="native-menu-list">
        <button onclick="renderConquestLog()"><span class="native-menu-icon">📖</span><span><strong>全国制覇帳</strong><small>制覇した自治体と地方別進捗</small></span><span>›</span></button>
        <button onclick="renderConquestMapView()"><span class="native-menu-icon">🗺️</span><span><strong>日本地図で見る</strong><small>都道府県ごとの達成状況</small></span><span>›</span></button>
        <button onclick="renderPrefectureCards()"><span class="native-menu-icon">🗾</span><span><strong>都道府県別進捗</strong><small>カードで詳しく確認</small></span><span>›</span></button>
        <button onclick="renderAchievementsPage()"><span class="native-menu-icon">🏅</span><span><strong>称号コレクション</strong><small>獲得した称号を確認</small></span><span>›</span></button>
      </div>
    </section>
    <section class="native-hub-section">
      <h2>プレイデータ</h2>
  ...
  `;
  footEl.textContent = 'この端末に保存されている記録';
  scrollToPageTop();
}

function renderNativePlayerRank(){
  if(!isNativeAppRuntime()) return renderOpening();
  stopOpeningMascotAnimation();
  stampsEl.innerHTML = '';
  pushNavState('nativePlayerRank');
  const playerRank = calculateNativePlayerRank(loadConquest());
  syncNativePlayerRankHeader(playerRank);
  const componentRows = Object.values(playerRank.components).map(component => {
    const rate = clampNumber(100 * component.score / component.maximum, 0, 100);
    return `
      <div class="native-player-component">
        <span><strong>${component.label}</strong><small>${component.score} / ${component.maximum}pt</small></span>
        <span class="native-player-component-bar"><i style="width:${rate}%"></i></span>
      </div>`;
  }).join('');
  const nextText = playerRank.nextRank
    ? `次の${playerRank.nextRank.grade}ランクまで あと${playerRank.nextRemaining}pt`
    : '最高ランクに到達しています！';
  const missingPrefs = Math.max(0, 47 - playerRank.reachedPrefectures);
  stage.innerHTML = `
    <header class="native-screen-heading">
      <span class="native-screen-icon" aria-hidden="true">🧭</span>
      <div><span>長期的な全国攻略の実績</span><h1>プレイヤーランク</h1><p>制覇数・難易度・質問効率・全国到達度で決まります</p></div>
    </header>
    <section class="native-player-rank-card native-player-rank-${playerRank.grade.toLowerCase()}">
      <small>現在のプレイヤーランク</small>
      <strong>${playerRank.grade}</strong>
      <h2>${escapeHtml(playerRank.title)}</h2>
      <p>${playerRank.score}pt</p>
    </section>
    <section class="native-player-next-card">
      <div><strong>${nextText}</strong><small>${escapeHtml(playerRank.advice)}</small></div>
      <div class="native-player-next-bar"><span style="width:${playerRank.progress}%"></span></div>
    </section>
    <div class="native-metric-grid native-player-metrics">
      <div class="native-metric"><strong>${playerRank.distinctCities.toLocaleString('ja-JP')}</strong><span>クリア自治体</span></div>
      <div class="native-metric"><strong>${playerRank.reachedPrefectures}</strong><span>到達都道府県</span></div>
      <div class="native-metric"><strong>${missingPrefs}</strong><span>未到達県</span></div>
    </div>
    <section class="native-hub-section native-player-components-section">
      <h2>ランク点の内訳</h2>
      <div class="native-player-components">${componentRows}</div>
    </section>
    <section class="native-player-rules-note">
      <strong>何度でも、後からでも追いつけます</strong>
      <p>同じ自治体は初回クリアの1か所として数え、再挑戦は自己ベストを更新したときだけ質問効率が伸びます。回答速度・プレイ日数・連続ログインは採点しません。</p>
    </section>
    <button class="link-btn" onclick="navigateBackOr(renderNativeRecordHub)">← 前の画面へ戻る</button>
  `;
  footEl.textContent = 'プレイヤーランクはこの端末の全国制覇帳から計算されます';
  scrollToPageTop();
}

function renderNativeRankHistory(){
  if(!isNativeAppRuntime()) return renderOpening();
  stopOpeningMascotAnimation();
  stampsEl.innerHTML = '';
  pushNavState('nativeRankHistory');
  const data = loadNativeRankData();
  const best = bestNativeRankResult(data);
  const rows = data.results.slice(0, 30).map(result => {
    ...
    const dateLabel = Number.isFinite(date.getTime())
      ...
      : '';
    return `
      <article class="native-rank-history-row">
        <span class="native-rank-history-grade native-rank-${result.grade.toLowerCase()}">${result.grade}</span>
        <span class="native-rank-history-main"><strong>${escapeHtml(result.cityName)}</strong><small>${escapeHtml(result.pref)} ・ ${dateLabel}</small></span>
        <span class="native-rank-history-score"><strong>${result.totalScore}点</strong><small>${result.questionCount}問／難易度 ${nativeDifficultyStars(result.difficultyLevel)}</small></span>
      </article>`;
  }).join('');
  stage.innerHTML = `
    <header class="native-screen-heading">
      <span class="native-screen-icon" aria-hidden="true">🔤</span>
      <div><span>1回ごとの上手さ</span><h1>クリア評価</h1><p>質問数と自治体の難しさで判定し、回答速度は使いません</p></div>
    </header>
    ${best ? `
      <section class="native-rank-best-card">
        <span>これまでの最高クリア評価</span>
        <strong class="native-rank-${best.grade.toLowerCase()}">${best.grade}</strong>
        <p>${escapeHtml(best.cityName)}（${escapeHtml(best.pref)}）・${best.totalScore}点・${best.questionCount}問</p>
      </section>` : `
      <section class="native-rank-empty-card">
        <strong>クリア評価はまだありません</strong>
        <p>全国版で正解すると、最初のクリア評価が記録されます。</p>
      </section>`}
    <section class="native-hub-section native-rank-history-section">
      <h2>最近の結果</h2>
      <div class="native-rank-history-list">${rows || '<p class="native-rank-history-empty">全国版のクリア評価がここに表示されます。</p>'}</div>
    </section>
    <button class="link-btn" onclick="navigateBackOr(renderNativeRecordHub)">← 前の画面へ戻る</button>
  `;
  footEl.textContent = '全国版のクリア評価はこの端末に保存されます';
  scrollToPageTop();
}
  ...
  stage.innerHTML = `
    ...
    <section class="native-settings-card">
      ...
      <label class="native-setting-toggle">
        <span><strong>匿名のプレイ結果を送信</strong><small>ゲーム改善のため、個人を特定しない結果を送ります</small></span>
    ...
    <section class="native-settings-card native-danger-zone">
      <h2>記録の削除</h2>
      <p>この端末の全国制覇帳を削除します。削除後は元に戻せません。</p>
...
window.oramachiNativeTabs = Object.freeze({
  open(tab){
    if(!isNativeAppRuntime()) return false;
    if(tab === 'home') renderOpening();
    else if(tab === 'play'){
      // 47都道府県一覧で選択中の「遊ぶ」を再タップした場合は、履歴を
      // 置き換えて二重の遊び方画面を作らず、通常の戻る操作と同じく1階層戻す。
      if(currentNavState.oramachiScreen === 'nativePrefecturePicker'){
        navigateBackOr(renderNativePlayHub);
      }else{
        renderNativePlayHub();
      }
    }
...
function renderOpening(){
  ...
  if(isNativeAppRuntime()){
    renderNativeHomeContent(totalCount, resumeCardHtml);
    scrollToPageTop();
    footEl.textContent = `対応エリア 日本全国 ${totalCount} 市区町村`;
    trackGaEvent('catchcopy_view');
    reportTodayVisitorOnce();
    return;
  }
...
function confirmDeleteConquest(){
  ...
  try{
    ...
    if(isNativeAppRuntime()) syncNativePlayerRankHeader(calculateNativePlayerRank(emptyConquestData()));
    const statusEl = document.getElementById('deleteConquestStatus');
    if(statusEl) statusEl.textContent = '削除しました。';
    if(isNativeAppRuntime() && currentNavState.oramachiScreen === 'nativeSettings'){
...
const PREFECTURES = PREFECTURE_MODE_PREFS;
...
function correct(isRight, overrideCity){
  ...
  if(isRight){
    ...
    // クリア評価はAndroid版の全国版だけで判定する。ブラウザ版と地域版の結果画面は
    // 従来どおりに保つ。回答速度は採点せず、質問数と自治体難易度だけを使う。
    const nativeRankResult = (isNativeAppRuntime() && currentMode === 'all')
      ? recordNativeRankResult(calculateNativeRankResult(guess, totalQuestions, [...answerLog], {
          wrongGuessCount: guessAttempts,
          firstClear: conquestResult.status === 'new',
    ...
    currentResult = {
      ...
      nativePlayerRankBefore, nativePlayerRankAfter,
    };
    markGameNavResult('deduction');
    ...
    const nativeClearBadges = nativeRankResult
      ? [
          nativeRankResult.firstClear ? '初見クリア' : '',
          nativeRankResult.personalBest ? '自己ベスト更新' : '',
        ].filter(Boolean)
      : [];
    const nativeRankHtml = nativeRankResult ? `
      <section class="native-rank-result" aria-label="今回のクリア評価 ${nativeRankResult.grade}">
        <span class="native-rank-result-grade native-rank-${nativeRankResult.grade.toLowerCase()}">${nativeRankResult.grade}</span>
        <span class="native-rank-result-copy">
          <small>今回のクリア評価</small>
          <strong>${nativeRankResult.totalScore}点</strong>
          <span>質問 ${nativeRankResult.questionScore}点 ・ 難易度 ${nativeDifficultyStars(nativeRankResult.difficultyLevel)} +${nativeRankResult.difficultyBonus}点</span>
          <span>${nativeRankResult.questionCount}問でクリア${nativeClearBadges.length ? ` ・ ${nativeClearBadges.join(' ・ ')}` : ''}</span>
          <span>回答速度は評価に含みません</span>
        </span>
      </section>` : '';
    const nativePlayerRankUp = nativePlayerRankBefore && nativePlayerRankAfter
      && nativePlayerRankAfter.rankIndex > nativePlayerRankBefore.rankIndex;
    const nativePlayerRankHtml = nativePlayerRankAfter ? `
      <button class="native-player-rank-result native-player-rank-${nativePlayerRankAfter.grade.toLowerCase()}" type="button" onclick="renderNativePlayerRank()">
        <span><small>プレイヤーランク</small><strong>${nativePlayerRankAfter.grade}</strong></span>
        <span><strong>${nativePlayerRankUp ? 'ランクアップ！' : escapeHtml(nativePlayerRankAfter.title)}</strong><small>${nativePlayerRankAfter.nextRank ? `次の${nativePlayerRankAfter.nextRank.grade}まであと${nativePlayerRankAfter.nextRemaining}pt` : '最高ランクに到達！'}</small></span>
        <span aria-hidden="true">›</span>
      </button>` : '';
