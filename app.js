...
const MIN_Q_BEFORE_EARLY_GUESS = 4; // 最低これだけ質問してから「早押し」判定を始める
...
const MODE_ONLY_KEYS = {
  ...
  tokyo: [
    'todai_campus','waseda_campus','keio_campus','sophia_campus','meiji_campus','aoyama_campus','rikkyo_campus','chuo_campus','hosei_campus','tus_campus','imperial_palace','diet_building','ginza','tokyo_tower_ward','tocho','tokyo_dome_ward','sensoji','skytree_ward','toyosu_market','haneda_ward','shibuya_crossing','shibamata_taishakuten','kasai_park','odakyu_line','keio_inokashira_line','tokyu_line','keikyu_line','seibu_line','tobu_line','keisei_line','tsukuba_express','rinkai_line','yurikamome','tokyo_monorail','toden_arakawa','nippori_toneri','tama_river','sumida_river','arakawa_river','edogawa_river','meguro_river','shakujii_river','borders_other_pref_tokyo','borders_kanagawa','borders_saitama','borders_chiba','ochanomizu_univ','gakushuin_univ','toyo_univ','komazawa_univ','seijo_univ','kokugakuin_univ','senshu_univ','jr_line','no_jr','yamanote_line','keihintohoku_line','chuo_rapid','chuo_sobu','joban_line','saikyo_line','tokyo_station','ikebukuro_station','ueno_station','shinagawa_station','akihabara_station','shimbashi_station','kinshicho_station','ogikubo_station','kamata_station','nippori_station','tsunagari_mayu_police','koiwa_or_kasairinkai_station','tora_san_home','tsubasa_hometown','tv_station_area','sazae_family','hachiko_area','yose_hall','sailor_moon_stage','tokiwa_so','godzilla_head','rakugo_stage','sanma_famous'
  ]
};
...
// 質問の「出やすさ」の調整。序盤は県境・大学などの効率が良い質問で普通に絞り込み、
// 候補がある程度絞れてきた終盤になってはじめて、遊び心のある質問(ご当地ネタ)を決め手として優先する。
// 鉄道系は出すぎるので軽く出にくく、大学系は出なさすぎるので少し出やすくする。
const HIGH_PRIORITY_KEYS = new Set([
  'tsunagari_mayu_police','tora_san_home','tsubasa_hometown','tv_station_area','sazae_family',
  'hachiko_area','yose_hall','sailor_moon_stage','tokiwa_so','godzilla_head','rakugo_stage','sanma_famous'
]);
const RAIL_KEYS = new Set([
  ...
  'jr_line','no_jr','yamanote_line','keihintohoku_line','chuo_rapid','chuo_sobu','joban_line','saikyo_line',
  'kintetsu','private_railway','monorail','tram','shinkansen','joetsu_shinkansen_station','hokuriku_shinkansen_station'
]);
const UNIV_KEYS = new Set([
  'todai_campus','waseda_campus','keio_campus','sophia_campus','meiji_campus','aoyama_campus',
  'rikkyo_campus','chuo_campus','hosei_campus','tus_campus','ochanomizu_univ','gakushuin_univ',
  'toyo_univ','komazawa_univ','seijo_univ','kokugakuin_univ','senshu_univ','imperial_university'
]);
...
const RAIL_PENALTY = 12;
const UNIV_BONUS = 12;

function priorityAdjust(k, diff, poolSize){
  ...
  if(RAIL_KEYS.has(k)) return diff + RAIL_PENALTY;
  if(UNIV_KEYS.has(k)) return Math.max(0, diff - UNIV_BONUS);
  return diff;
}
...
function entropyPick(){
  ...
  const prelim = candidateQuestions.map(k=>{
    const yes = topCities.filter(c=>c.tags[k]).length;
    const no = topCities.length - yes;
    return { k, diff: priorityAdjust(k, Math.abs(yes - no), topCities.length) };
  ...
  const scored = shortlist.map(k=>{
    ...
    return { k, minimax: priorityAdjust(k, Math.max(worstYes, worstNo), topCities.length) };
  });
...
function answer(key, val){
  if(val === true){
    ...
    // 「はい」と答えた時点でそれが正解だと確定できるので、飛び級で正解画面まで進める。
  ...
  if(val !== null){
    ...
    // こうすることで、1問だけ勘違いして答えてしまっても正解がいきなり消えることがなくなる。
    scorePool.forEach(e => {
      if(e.city.tags[key] === val) e.score += MATCH_BONUS;
